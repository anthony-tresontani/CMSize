...
from cms_processor import add_content_type, add_content, add_cms_contents, apply_layout, validate_layout
...
class ContentTypeTest(TestCase):
    ...
    def test_get_content_for_page(self):
        ...
        assert_that(add_cms_contents(user="fangiot", page="<div id='infos'></div>"), is_(u'<div id="infos"><p>Something happened hÉ</p></div>'))
        assert_that(add_cms_contents(user="fangiot", page='<div id="news">my news</div>'), is_(u'<div id="news">my news</div>'))
...
class CMSTest(TestCase):
    ...
    def test_add_a_newslist(self):
        ...
        cms_page = add_cms_contents("fangiot", page)
